...
:hInst(GetModuleHandle(nullptr))
{
   ...
   // wc.hIcon = static_cast<HICON>(LoadImage(hInst, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, 512, 512, 0));
    wc.hCursor = nullptr;
    wc.hbrBackground = nullptr;
    wc.lpszMenuName = nullptr;
    wc.lpszClassName = GetName();
   // wc.hIconSm = static_cast<HICON>(LoadImage(hInst, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, 512, 512, 0));;
    RegisterClassEx(&wc);
}
...
{
    if (SetWindowText(hWnd, ttitle.c_str()) == 0)
    {
        throw new std::exception();
    }
}
